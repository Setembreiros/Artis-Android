...
class PostMapper: Mapper<Post, CreatePostRequestApi> {
    override fun map(model: Post): CreatePostRequestApi {
        return CreatePostRequestApi(username = model.metadata.username, title = model.metadata.title, description = model.metadata.description, fileType = model.metadata.fileType, type = model.metadata.type)
    }
}