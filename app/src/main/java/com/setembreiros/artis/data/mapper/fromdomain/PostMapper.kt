...
class PostMapper: Mapper<Post, CreatePostRequestApi> {
    override fun map(model: Post): CreatePostRequestApi {
        return CreatePostRequestApi(username = model.metadata.username, title = model.metadata.title, description = model.metadata.description, type = model.metadata.type, size = model.metadata.size, hasThumbnail = model.metadata.type != Constants.ContentType.AUDIO)
    }
}